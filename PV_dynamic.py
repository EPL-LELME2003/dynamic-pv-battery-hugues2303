...
time_step = len(load) #number of hours
...
charge_time = 4  # hours to charge fully the battery -> link between power and energy
...
model.P_pv_prod = Var(range(time_step), domain=NonNegativeReals)
model.P_batt_in = Var(range(time_step), domain=NonNegativeReals)
model.P_batt_out = Var(range(time_step), domain=NonNegativeReals)
model.SOC_time = Var(range(time_step), domain=NonNegativeReals)
...
model.P_pv_production_constraint = Constraint(range(time_step), rule=P_pv_production_rule)
...
model.battery_soc_constraint = Constraint(range(time_step), rule=battery_soc_rule)

def battery_capacity_rule(model, t):
    return model.SOC_time[t] <= model.SOC_max
model.battery_capacity_constraint = Constraint(range(time_step), rule=battery_capacity_rule)

def load_balance_rule(model, t):
    return load[t] == model.P_pv_prod[t] + model.P_batt_out[t] - model.P_batt_in[t]
model.load_balance_constraint = Constraint(range(time_step), rule=load_balance_rule)

def charge_in_max_rule(model, t):
    return model.P_batt_in[t] <= model.SOC_max/charge_time
model.charge_in_max_constraint = Constraint(range(time_step), rule=charge_in_max_rule)

def charge_out_max_rule(model, t):
    return model.P_batt_out[t] <= model.SOC_max/charge_time
model.charge_out_max_constraint = Constraint(range(time_step), rule=charge_out_max_rule)
...
plt.plot([model.P_pv_prod[t]() for t in range(time_step)], label='PV Production (kWh)')
plt.plot([model.SOC_time[t]() for t in range(time_step)], label='Battery SOC_time (kWh)')
